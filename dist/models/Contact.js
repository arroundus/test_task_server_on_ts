...
class Contact {
    constructor(cid, name, surname, post) {
        this.cid = cid;
        this.name = name;
    ...
    static async getContactById(cid) {
        const query = `
      SELECT c.cid, c.name, c.surname, c.post,
    ...
    static async createContact(name, surname, post) {
        const query = `
    ...
    static async updateContact(cid, name, surname, post) {
        const query = `
      UPDATE contacts 
